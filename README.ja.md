...
- **タイムライン** — ホームタイムライン、カスタムフィード切り替え（iOS はスワイプ対応）、自動更新（間隔設定可）、プルリフレッシュ、リロードボタン、無限スクロール
...
- **プロフィール** — 投稿/返信/メディア/いいねタブ、ピン留め投稿、フォロー管理、フォロワー/フォロー中一覧、プロフィール内検索、説明文リンクのクリック対応
...
- **ダイレクトメッセージ・グループチャット** — 1対1会話とグループチャット（最大50名）に対応。グループ作成、招待リンク管理、参加申請承認、メンバー管理、ロック/解除。絵文字リアクション、ユーザー検索履歴付き新規会話作成
- **プロフィール QR コード** — 自分の Bluesky プロフィールの QR コードを生成・共有。リンクコピー、OS 共有シート、写真ライブラリへの保存に対応
- **コンテンツモデレーション** — ラベル別フィルタ（hide/warn/ignore）、成人向けコンテンツ切り替え、投稿通報
- **設定** — テーマ、文字サイズ（4段階）、投稿言語、自動更新間隔、via 表示、Claude API キー（ALT テキスト自動生成）
...
- **バックグラウンド更新** — 定期バックグラウンドポーリングと未読通知のローカルプッシュ通知（iOS）、フォアグラウンドポーリングと macOS 通知（Mac Catalyst）
- **キーボードショートカット（macOS）** — Cmd+N（新規投稿）、Cmd+Return（投稿送信）、Cmd+R（リロード）、Cmd+1〜5（タブ切替）
...
- [kazahana](https://github.com/osprey74/kazahana) — デスクトップ版 (Windows)
