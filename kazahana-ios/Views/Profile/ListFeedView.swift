// ListFeedView.swift
// kazahana-ios
// リストフィード表示画面（app.bsky.feed.getListFeed）
...
struct ListFeedView: View {
        ...
        Group {
            if isLoading && posts.isEmpty {
                ...
                VStack(spacing: 12) {
                    ...
                    Text(String(localized: "list.noPosts"))
                        .font(.subheadline)
                        .foregroundStyle(.secondary)
                }
                .frame(maxWidth: .infinity, maxHeight: .infinity)
    ...
    private func loadInitial() async {
        ...
        hasMore = true
        do {
            let response = try await feedService.getListFeed(listURI: list.uri, limit: 50)
            posts = response.feed
    ...
    private func loadMore() async {
        ...
        isLoading = true
        do {
            let response = try await feedService.getListFeed(listURI: list.uri, limit: 50, cursor: currentCursor)
